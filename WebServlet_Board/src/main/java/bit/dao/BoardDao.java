...
import bit.utils.ConnectionPoolHelper;
...
public class BoardDao {
	...
	public BoardDao() {
        try {
            ...
            throw new RuntimeException("DBCP Lookup 실패", e);
        }
    }
		...
		*/
		...
		try {
			...
			String sql = "SELECT * FROM (" +
					    "   SELECT ROWNUM rn, b.idx, b.writer, b.email, b.homepage, b.pwd, b.subject, b.content, " +
					    "          b.writedate, b.readnum, b.filename, b.filesize, b.refer, b.depth, b.step, " +
					    "          (SELECT COUNT(*) FROM reply r WHERE r.idx_fk = b.idx) AS replycount " +
					    "   FROM (SELECT * FROM jspboard ORDER BY refer DESC, step ASC) b " +
					    "   WHERE ROWNUM <= ? " +
					    ") WHERE rn >= ?";
			pstmt = conn.prepareStatement(sql);
			//공식같은 로직
	...
	public boolean isWriterAvailable(String writer) {
        String sql = "SELECT COUNT(*) FROM jspboard WHERE writer=?";
